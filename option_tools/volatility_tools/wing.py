...
class Wing(VolatilityModelBase):
    ...
    def volatility_curve(self, X_list,
        ...
        x_to_X = [f_syn * math.exp(i * time_weighting_effect)
                  for i in [x0, x1, x2, x3]]

        #         return value:
        return dict({'f_syn': f_syn,
                     'f_atm': f_atm,
                     'f_ref': f_ref,
                     'f_syn_to_x': f_syn_to_x,
                     'f_atm_to_x': f_atm_to_x,
                     'f_ref_to_x': f_ref_to_x,
                     'vol_curr': vol_curr,
